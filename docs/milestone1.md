...
But differentiating an arbitrary function `<&#8477><sup>n</sup> <&rarr> <&#8477><sup>m</sup>` is generally not an easy task. In case the function can be expressed as a composition of differentiable elementary functions (which in most cases is true), Automatic Differentiation can help. 
